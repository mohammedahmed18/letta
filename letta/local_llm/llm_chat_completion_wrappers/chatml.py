...
class ChatMLInnerMonologueWrapper(LLMChatCompletionWrapper):
    ...
    def _compile_system_message(self, system_message, functions, function_documentation=None) -> str:
        ...
        # Using list for memory efficiency and speed
        prompt_parts = []
        ...
        if function_documentation is not None:
            ...
            prompt_parts.append(function_documentation)
        else:
            prompt_parts.append(self._compile_function_block(functions))
    ...
    def _compile_assistant_message(self, message) -> str:
        ...
        if "function_call" in message and message["function_call"]:
            ...
            prompt_parts.append(inner_thoughts)
        return "".join(prompt_parts)
    ...
    def _compile_user_message(self, message) -> str:
        ...
        if self.simplify_json_content:
            try:
                user_msg_str = json_loads(content)["message"]
            except Exception:
                user_msg_str = content
        else:
            # Otherwise just dump the full json
            try:
                user_msg_json = json_loads(content)
    ...
    def _compile_function_response(self, message) -> str:
        ...
        try:
            ...
            function_return_dict = json_loads(content)
            function_return_str = json_dumps(function_return_dict, indent=0)
        except Exception:
            function_return_str = content
    ...
    def chat_completion_to_prompt(self, messages, functions, first_message=False, function_documentation=None):
        ...
        mr_enum_vals = [role.value for role in MessageRole]

        append = result.append  # localize for speed
        allow_custom_roles = self.allow_custom_roles
        use_system_role_in_user = self.use_system_role_in_user
        allow_function_role = self.allow_function_role
        no_function_role_role = self.no_function_role_role
        no_function_role_prefix = self.no_function_role_prefix
        _compile_user_message = self._compile_user_message
        _compile_assistant_message = self._compile_assistant_message
        _compile_system_message = self._compile_system_message
        _compile_function_response = self._compile_function_response

        for message in messages[1:]:
            role = message["role"]
            # check that message["role"] is a valid option for MessageRole
            assert role in mr_enum_vals
            if role == "user":
                if allow_custom_roles and "name" in message:
                    role_str = message["name"].strip().lower()
                else:
                    role_str = role
                msg_str = _compile_user_message(message)
                # Use system role for some user messages optionally
                if use_system_role_in_user:
                    try:
                ...
                if allow_custom_roles and "name" in message:
                    ...
                    role_str = role
                msg_str = _compile_assistant_message(message)
                append(f"\n<|im_start|>{role_str}\n{msg_str.strip()}<|im_end|>")
            elif role == "system":
                role_str = "system"
                msg_str = _compile_system_message(
                ...
                msg_str = _compile_function_response(message)
                if allow_function_role:
                    role_str = role
                    append(f"\n<|im_start|>{role_str}\n{msg_str.strip()}<|im_end|>")
                else:
                    role_str = no_function_role_role
                    func_resp_prefix = no_function_role_prefix
                    append(
        ...
        if self.include_assistant_prefix:
            ...
            if getattr(self, "supports_first_message", False) and first_message:
                if self.assistant_prefix_extra_first_message:
                    result.append(self.assistant_prefix_extra_first_message)
            else:
                if self.assistant_prefix_extra:
                    result.append(self.assistant_prefix_extra)
